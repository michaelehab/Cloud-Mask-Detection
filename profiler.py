...
def count_convtranspose2d(m, x, y):
    ...
    kernel_add = kh * kw * cin - 1
    bias_ops  = 1 if m.bias is not None else 0
    ops_per_element = kernel_mul + kernel_add + bias_ops

    num_out_elements = y.numel()
    total_ops = num_out_elements * ops_per_element
    m.total_ops += torch.Tensor([int(total_ops)])
...
def main(args):
    ...
    model.load_state_dict(torch.load(args.model_path))  # load weights only
...
if __name__ == "__main__":
    ...
    parser.add_argument("model_path", help="model to profile")
